    ...
    <div className="dashboard d-flex flex-row">
      ...
      <div className="container text-center">
        <div className="shadow-none p-3 mt-3 mb-5 bg-body rounded main-title">
          <h2 className='fs-1 fw-bold text-dark'>انيستا باى (INSTA PAY)</h2>
        </div>
        <div className="d-flex flex-row justify-content-between">
          <Link to="/dash/elfath/instaPay/add-instaPay">
        ...
        <table className="table table-striped table-hover">
          <thead>
            <tr>
              ...
              <th scope="col">الحد الشهري (400,000)</th>
          ...
          <tbody>
            {!loading && cashVoda?.map((item, index) => (
              <tr key={index}>
                ...
                <td>
                  <Link
                    to={`/dash/elfath/instaPay/update-instaPay/${item._id}`}
                    state={{ item: item }}
                  >
                    <button className="btn btn-outline-success mx-2 px-4">تعديل</button>
                  </Link>
                  <Link
                    to={`/dash/elfath/instaPay/details-instaPay/${item._id}`}
                    state={{ item: item }}
                  >
